...
Transparent PowerShell Edition - Version 0.21
===============================================================================
...
function Write-SchedulerStatus {
    ...
    try {
        ...
        if ($null -ne $ArmedWakeUtc) {
            $wakeText = $ArmedWakeUtc.Value.ToString("yyyy-MM-dd HH:mm:ss")
        }
...
function Show-MainMenu {
    while ($true) {
        ...
        Write-Host "Transparent PowerShell Edition v0.21"
        Write-Host "======================================================================"
        Write-Host ""
...
function Start-OutpostScheduler {
    ...
    try {
        ...
        while ($true) {
            ...
            $timeMatches = (
                $nowUtc.Hour -eq $scheduledHour -and
                $nowUtc.Minute -eq $scheduledMinute
            )

            if ($timeMatches -and $lastRunUtcDate -ne $todayUtc) {
