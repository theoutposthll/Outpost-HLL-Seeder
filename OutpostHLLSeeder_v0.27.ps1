...
Transparent PowerShell Edition - Version 0.27
===============================================================================
...
function Open-ActivityLog {
    # The known-default HLL reference is supplied by The Outpost, not user data.
    # On every install/update from a downloaded release, always copy/refresh it
    # into the installed setup_files folder. User config and backups are handled
    # separately and are never overwritten by this reference-file update.
    if (Test-Path -LiteralPath $sourceDefaultSettingsReference) {
        Copy-Item -LiteralPath $sourceDefaultSettingsReference `
            -Destination $HllSettingsDefaultReference `
            -Force
    }

    if (-not (Test-Path -LiteralPath $HllSettingsDefaultReference)) {
        throw "HLL default-settings reference was not installed successfully."
    }

    Write-Host "Installed HLL default-settings reference:"
    Write-Host "  $HllSettingsDefaultReference"

    if (
        (Test-Path -LiteralPath $legacyReferenceFile) -and
        ([System.IO.Path]::GetFullPath($legacyReferenceFile) -ine [System.IO.Path]::GetFullPath($HllSettingsDefaultReference))
    ) {
        Remove-Item -LiteralPath $legacyReferenceFile -Force -ErrorAction SilentlyContinue
    }
...
function Show-MainMenu {
    while ($true) {
        ...
        Write-Host "Transparent PowerShell Edition v0.27"
        Write-Host "======================================================================"
        Write-Host ""
...
function Install-OutpostSeeder {
    ...
    # v0.27 installed layout migration.
