...
Transparent PowerShell Edition - Version 0.31
===============================================================================
...
$InstalledSetupDirectory = Join-Path $AppDirectory "setup_files"
$ConfigFile = Join-Path $InstalledSetupDirectory "config.txt"
...
$HllSettingsDefaultReference = Join-Path $InstalledSetupDirectory "HLL_GameUserSettings_DEFAULT_REFERENCE.ini"
$HllSettingsBackupFile = Join-Path $InstalledSetupDirectory "GameUserSettings.backup.ini"
$HllSettingsPreviousBackupFile = Join-Path $InstalledSetupDirectory "GameUserSettings.backup.previous.ini"
$DefaultHllSettingsFile = Join-Path $env:LOCALAPPDATA "HLL\Saved\Config\WindowsNoEditor\GameUserSettings.ini"

# Capture the launch/source package paths immediately. This is deliberately done
# before setup can copy/relaunch the script from %LOCALAPPDATA%.
$LaunchSourceDirectory = Split-Path -Parent $PSCommandPath
$LaunchSetupDirectory = Join-Path $LaunchSourceDirectory "setup_files"
$LaunchConfigFile = Join-Path $LaunchSetupDirectory "config.txt"
$LaunchDefaultSettingsReference = Join-Path $LaunchSetupDirectory "HLL_GameUserSettings_DEFAULT_REFERENCE.ini"

$StartupDirectory = Join-Path $env:APPDATA "Microsoft\Windows\Start Menu\Programs\Startup"
$StartupFile = Join-Path $StartupDirectory "Outpost HLL Auto-Seeder.vbs"
$LegacyStartupFile = Join-Path $StartupDirectory "Outpost HLL Auto-Seeder.cmd"
...
function Read-Config {
    ...
    if ($values["PROTECT_HLL_SETTINGS"] -notmatch '^(?i:true|false)$') {
        throw "PROTECT_HLL_SETTINGS must be true or false."
    }
    ...
    return @{
        ...
        ProtectHllSettings = [System.Convert]::ToBoolean($values["PROTECT_HLL_SETTINGS"])
        HllSettingsFile = [string]$values["HLL_SETTINGS_FILE"]
    }
}
...
function Resolve-HllSettingsFile {
    param([hashtable]$Config)

    $configured = ([string]$Config.HllSettingsFile).Trim()

    if ($configured) {
        $expanded = [Environment]::ExpandEnvironmentVariables($configured)

        if (-not [IO.Path]::IsPathRooted($expanded)) {
            Write-Log "WARNING: HLL_SETTINGS_FILE must be a full path when specified. Settings protection skipped."
            return $null
        }
    ...
    return $DefaultHllSettingsFile
}


function Get-NormalizedIniContent {
    param([string]$Path)

    # Reference-file comments and blank lines are intentionally ignored.
    # Remaining lines are trimmed and compared in order.
...
function Test-HllSettingsFileLooksValid {
    param([string]$Path)
    ...
    try {
        ...
        $content = Get-NormalizedIniContent -Path $Path
        if (-not $content) {
            return $false
        }

        # A usable HLL settings file must contain the main game-user-settings
        # section and a few core keys. This prevents an empty/truncated file
        # from ever replacing or being used as a local recovery backup.
        $requiredLines = @(
        ...
        foreach ($requiredLine in $requiredLines) {
            if ($requiredLine.EndsWith('=')) {
                if ($content -notmatch ('(?m)^' + [regex]::Escape($requiredLine))) {
                    return $false
                }
            }
            elseif ($content -notmatch ('(?m)^' + [regex]::Escape($requiredLine) + '$')) {
                return $false
            }
        }
...
function Test-IniMatchesDefaultReference {
    param(
        [string]$CandidatePath,
        [string]$ReferencePath
    )

    if (-not (Test-HllSettingsFileLooksValid -Path $CandidatePath)) {
        return $false
    }

    $candidateLines = @(Get-NormalizedIniContent -Path $CandidatePath -ErrorAction Stop -split "`n")
    $referenceLines = @(Get-NormalizedIniContent -Path $ReferencePath -ErrorAction Stop -split "`n")

    # These values are machine-, session-, or game-version-specific and can
    # legitimately differ in a freshly reset file. They must not prevent reset
    # detection. Extra keys added by a future HLL update are also ignored.
    $ignoredKeys = @{
        'HLLSettingsVersion' = $true
        'LastSeenEULAVersion' = $true
        'HLLCosmeticsVersion' = $true
        'HintsVersion' = $true
        'LastAudioInputDevice' = $true
        'LastAudioOutputDevice' = $true
        'ResolutionSizeX' = $true
        'ResolutionSizeY' = $true
        'LastUserConfirmedResolutionSizeX' = $true
        'LastUserConfirmedResolutionSizeY' = $true
        'WindowPosX' = $true
        'WindowPosY' = $true
        'LastUserConfirmedDesiredScreenWidth' = $true
        'LastUserConfirmedDesiredScreenHeight' = $true
        'LastRecommendedScreenWidth' = $true
        'LastRecommendedScreenHeight' = $true
        'LastCPUBenchmarkResult' = $true
        'LastGPUBenchmarkResult' = $true
        'LastCPUBenchmarkSteps' = $true
        'LastGPUBenchmarkSteps' = $true
        'LastGPUBenchmarkMultiplier' = $true
    }

    $candidateSet = @{}
    foreach ($line in $candidateLines) {
        $candidateSet[$line] = $true
    }

    foreach ($line in $referenceLines) {
        if ($line -match '^([^=]+)=') {
            $key = $Matches[1]
            if ($ignoredKeys.ContainsKey($key)) {
                continue
            }
        }

        if (-not $candidateSet.ContainsKey($line)) {
            return $false
        }
    }
...
function Ensure-InitialHllSettingsBackup {
    param([hashtable]$Config)

    if (-not $Config.ProtectHllSettings) {
        Write-Log "HLL settings protection is disabled. Initial backup check skipped."
        return
    }

    # Never replace an existing local recovery point during install/update/repair.
    if (Test-Path -LiteralPath $HllSettingsBackupFile) {
        Write-Log "Existing HLL settings backup found. Initial backup check not needed."
        return
    }

    try {
        $liveSettingsFile = Resolve-HllSettingsFile -Config $Config

        if (-not $liveSettingsFile) {
            return
        }

        if (-not (Test-Path -LiteralPath $HllSettingsDefaultReference)) {
            Write-Log "WARNING: HLL settings default reference is missing. Initial backup check skipped."
            return
        }

        if (-not (Test-Path -LiteralPath $liveSettingsFile)) {
            Write-Log "HLL settings file was not found: $liveSettingsFile. Initial backup check skipped."
            return
        }

        if (-not (Test-HllSettingsFileLooksValid -Path $liveSettingsFile)) {
            Write-Log "WARNING: HLL GameUserSettings.ini is empty or incomplete. Initial backup was not created."
            return
        }

        $liveLooksDefault = Test-IniMatchesDefaultReference `
            -CandidatePath $liveSettingsFile `
            -ReferencePath $HllSettingsDefaultReference

        if ($liveLooksDefault) {
            Write-Log "HLL GameUserSettings.ini currently matches the known-default reference. No initial backup was created."
            return
        }

        Copy-Item -LiteralPath $liveSettingsFile `
            -Destination $HllSettingsBackupFile `
            -Force

        Write-Log "Created initial local backup of the user's HLL GameUserSettings.ini."
    }
    catch {
        # Initial backup protection must never make setup fail.
        Write-Log "WARNING: Initial HLL settings backup could not be created: $($_.Exception.Message)"
        Write-ErrorLog -Context "Creating initial HLL GameUserSettings.ini backup" -ErrorRecord $_
    }
}


function Protect-HllGameUserSettings {
    param([hashtable]$Config)

    if (-not $Config.ProtectHllSettings) {
        Write-Log "HLL settings protection is disabled."
        return
    }
    ...
    try {
        ...
        if (-not (Test-Path -LiteralPath $HllSettingsDefaultReference)) {
            Write-Log "WARNING: HLL settings default reference is missing. Settings protection skipped."
            return
        }

        if (-not (Test-Path -LiteralPath $liveSettingsFile)) {
            Write-Log "HLL settings file was not found: $liveSettingsFile. Settings protection skipped."
            return
        }

        $liveIsValid = Test-HllSettingsFileLooksValid -Path $liveSettingsFile
        $liveLooksDefault = $false
        ...
        if ((-not $liveIsValid) -or $liveLooksDefault) {
            $restoreSource = $null

            # Prefer the newest local backup, but never restore a backup that
            # itself matches the known-default reference.
            if (Test-Path -LiteralPath $HllSettingsBackupFile) {
                $backupIsValid = Test-HllSettingsFileLooksValid -Path $HllSettingsBackupFile

                if ($backupIsValid) {
                    $backupLooksDefault = Test-IniMatchesDefaultReference `
                        -CandidatePath $HllSettingsBackupFile `
                        -ReferencePath $HllSettingsDefaultReference

                    if (-not $backupLooksDefault) {
                        $restoreSource = $HllSettingsBackupFile
                    }
                }
            }

            # Keep one previous generation as an extra safety net.
            if (-not $restoreSource -and (Test-Path -LiteralPath $HllSettingsPreviousBackupFile)) {
                $previousIsValid = Test-HllSettingsFileLooksValid -Path $HllSettingsPreviousBackupFile

                if ($previousIsValid) {
                    $previousLooksDefault = Test-IniMatchesDefaultReference `
                        -CandidatePath $HllSettingsPreviousBackupFile `
                        -ReferencePath $HllSettingsDefaultReference

                    if (-not $previousLooksDefault) {
                        $restoreSource = $HllSettingsPreviousBackupFile
                    }
                }
            }

            if ($restoreSource) {
                Copy-Item -LiteralPath $restoreSource -Destination $liveSettingsFile -Force

                if ($liveIsValid) {
                    Write-Log "HLL GameUserSettings.ini matched the known-default reference. Restored the user's local backup."
                }
                else {
                    Write-Log "HLL GameUserSettings.ini was empty or incomplete. Restored the user's local backup."
                }
            }
            else {
                if ($liveIsValid) {
                    Write-Log "HLL GameUserSettings.ini matched the known-default reference, but no valid non-default local backup exists. Nothing was changed."
                }
                else {
                    Write-Log "WARNING: HLL GameUserSettings.ini is empty or incomplete, but no valid non-default local backup exists. Nothing was changed."
                }
            }
        ...
        # The live file does not match the known-default reference, so treat it
        # as the user's current settings and refresh the local backup.
        if (Test-Path -LiteralPath $HllSettingsBackupFile) {
            try {
                $currentBackup = Get-NormalizedIniContent -Path $HllSettingsBackupFile
                $liveContent = Get-NormalizedIniContent -Path $liveSettingsFile

                if ($currentBackup -cne $liveContent) {
                    Copy-Item -LiteralPath $HllSettingsBackupFile `
                        -Destination $HllSettingsPreviousBackupFile `
                        -Force
                }
            }
            catch {
                # Previous-generation backup is best-effort only.
            }
        }
        ...
        Write-Log "HLL GameUserSettings.ini differs from the known-default reference. Local settings backup refreshed."
    }
    catch {
        # Settings protection must never block a seeding run.
        Write-Log "WARNING: HLL settings protection could not complete: $($_.Exception.Message)"
        Write-ErrorLog -Context "Protecting HLL GameUserSettings.ini" -ErrorRecord $_
    }
}
...
function Stop-Hll {
    ...
    Write-Log "Requesting a normal Hell Let Loose shutdown (PID $processId)."
    ...
    try {
        ...
        if ($closeRequested) {
            Write-Log "Normal close request sent. Waiting up to 30 seconds for Hell Let Loose to exit."

            $deadline = (Get-Date).AddSeconds(30)
            while ((Get-Date) -lt $deadline) {
                if (-not (Test-HllRunning)) {
                    Write-Log "Hell Let Loose closed normally."
                    return $true
                }
            ...
            Write-Log "Hell Let Loose did not exit within 30 seconds. Falling back to forced termination."
        }
        else {
            Write-Log "Normal close request could not be sent. Falling back to forced termination."
        }

        Stop-Process -Id $processId -Force -ErrorAction Stop
        Write-Log "Hell Let Loose was force-closed successfully."
        return $true
    }
    catch {
...
function Invoke-SeedingCheck {
    ...
    if ($PreflightOnly) {
        Write-Log "Scheduled seeding preflight passed. Server requires seeding."
        $result.Reason = "Seeding required"
        return $result
    }

    # Re-check immediately before launch so a game started after the earlier
    # eligibility checks is still left completely alone.
    if (Test-HllRunning) {
        Write-Log "Hell Let Loose started before the Seeder launch step. Seeder will not interfere with the existing game session."
    ...
    $otherSteamGame = Get-OtherSteamGameProcess
    if ($null -ne $otherSteamGame) {
        Write-Log "Another Steam game started before the Seeder launch step: $($otherSteamGame.ProcessName) (PID $($otherSteamGame.ProcessId))."
        Write-Log "Seeder cancelled. The existing game will not be interrupted."
        $result.ShouldLaunch = $false
        $result.Reason = "Another Steam game is running"
        return $result
    ...
    # Before EVERY Seeder-controlled HLL launch, protect the user's plain-text
    # settings file. This same path is used by both the scheduled timer and
    # menu option 1 ("Run seeder now").
    # Failure here is logged but never blocks the seeding run.
    Protect-HllGameUserSettings -Config $config
    ...
    if ($result.HllClosedBySeeder) {
        # HLL can write GameUserSettings.ini during shutdown. Wait until every
        # HLL process has fully exited, then run the same settings-protection
        # check used before launch so a reset/blank file is restored immediately.
        Write-Log "Waiting for Hell Let Loose to finish shutting down before checking settings."
        while (Test-HllRunning) {
            Start-Sleep -Seconds 1
        }

        Write-Log "Hell Let Loose has fully stopped. Checking GameUserSettings.ini again."
        Protect-HllGameUserSettings -Config $config
...
function Get-SourceBatPath {
    ...
    #   OutpostHLLSeeder_v0.29.bat
    #   OutpostHLLSeeder_v0.29.ps1
    #
...
function Show-MainMenu {
    while ($true) {
        ...
        Write-Host "Transparent PowerShell Edition v0.29"
        Write-Host "======================================================================"
        Write-Host ""
        ...
        Write-Host "9. Exit"
        Write-Host ""
        Write-Host "The scheduler runs separately in the background. You can safely close this MENU window at any time."
        Write-Host ""
...
function Show-LiveFileLocations {
    ...
    Write-Host ""
    Write-Host "Only the installed setup_files configuration above is used by the running seeder."
    Write-Host "The live installed configuration is stored under setup_files\config.txt."
}
...
function Install-OutpostSeeder {
    ...
    New-Item -ItemType Directory -Force -Path $AppDirectory | Out-Null
    New-Item -ItemType Directory -Force -Path $InstalledSetupDirectory | Out-Null
    New-Item -ItemType Directory -Force -Path $StartupDirectory | Out-Null

    # Install the Outpost-supplied reference from an extracted release.
    # When Option 6 is run from the already-installed copy, source and
    # destination are the same file, so deliberately skip that self-copy.
    if (Test-Path -LiteralPath $LaunchDefaultSettingsReference) {
        $referenceSourceFull = [System.IO.Path]::GetFullPath($LaunchDefaultSettingsReference)
        $referenceDestinationFull = [System.IO.Path]::GetFullPath($HllSettingsDefaultReference)

        if ($referenceSourceFull -ine $referenceDestinationFull) {
            Copy-Item -LiteralPath $LaunchDefaultSettingsReference `
                -Destination $HllSettingsDefaultReference `
                -Force
        }
        else {
            Write-Host "Installed HLL default-settings reference is already the active source; skipping self-copy."
        }

        if (-not (Test-Path -LiteralPath $HllSettingsDefaultReference)) {
            throw "Failed to install HLL default-settings reference into setup_files."
        }
    }

    # Use the paths captured when this script process originally started.
    # This prevents setup/self-copy logic from accidentally looking for release
    # files under %LOCALAPPDATA% instead of the extracted download folder.
    $sourceDirectory = $LaunchSourceDirectory
    $sourceBat = Get-SourceBatPath
    $sourceSetupDirectory = $LaunchSetupDirectory
    $sourceConfig = $LaunchConfigFile
    $sourceDefaultSettingsReference = $LaunchDefaultSettingsReference

    if (-not (Test-Path $sourceBat)) {
        throw "Matching BAT file not found. Expected beside this file: $sourceBat"
    }

    # v0.29 installed layout migration.
    # Older releases stored config/reference/backups directly in AppDirectory.
    # Move those files into setup_files without overwriting an already-migrated
    # destination. This preserves all user choices and local HLL backups.
    $legacyConfigFile = Join-Path $AppDirectory "config.txt"
    $legacyReferenceFile = Join-Path $AppDirectory "HLL_GameUserSettings_DEFAULT_REFERENCE.ini"
    $legacyBackupFile = Join-Path $AppDirectory "GameUserSettings.backup.ini"
    $legacyPreviousBackupFile = Join-Path $AppDirectory "GameUserSettings.backup.previous.ini"

    foreach ($pair in @(
        @($legacyConfigFile, $ConfigFile),
        @($legacyBackupFile, $HllSettingsBackupFile),
        @($legacyPreviousBackupFile, $HllSettingsPreviousBackupFile)
    )) {
        $oldPath = $pair[0]
        $newPath = $pair[1]
        ...
        if ((Test-Path -LiteralPath $oldPath) -and -not (Test-Path -LiteralPath $newPath)) {
            ...
            Write-Host "Migrated $(Split-Path $oldPath -Leaf) into installed setup_files."
        }
    }
    ...
    if (-not $configAlreadyExists -and -not (Test-Path $sourceConfig)) {
        throw "First-time setup requires setup_files\config.txt in the downloaded seeder folder."
    }

    if (
        -not (Test-Path -LiteralPath $HllSettingsDefaultReference) -and
        -not (Test-Path -LiteralPath $sourceDefaultSettingsReference)
    ) {
        throw "Setup requires setup_files\HLL_GameUserSettings_DEFAULT_REFERENCE.ini."
    }
    ...
    # Establish a first HLL settings recovery point as soon as setup/repair has
    # a valid installed config and known-default reference. Existing backups are
    # never overwritten here.
    try {
        $settingsProtectionConfig = Read-Config
        Ensure-InitialHllSettingsBackup -Config $settingsProtectionConfig
    }
    catch {
        Write-Log "WARNING: Could not perform initial HLL settings backup check: $($_.Exception.Message)"
        Write-ErrorLog -Context "Initial HLL settings backup check during setup" -ErrorRecord $_
    }
    ...
    # Windows Startup uses a tiny VBScript launcher rather than CMD.
    # WScript.Shell.Run windowStyle=0 hides PowerShell, and waitOnReturn=False
    # lets the VBS process exit immediately while the scheduler continues.
    $escapedPs1 = $InstalledPs1.Replace('"', '""')
...
shell.CurrentDirectory = "$escapedAppDirectory"
command = "powershell.exe -NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -File ""$escapedPs1"" -Scheduler"
...
"@
    ...
    # Remove the old v0.25-and-earlier CMD Startup launcher so upgrades do not
    # leave two automatic scheduler launchers behind.
    if (Test-Path $LegacyStartupFile) {
        Remove-Item $LegacyStartupFile -Force -ErrorAction SilentlyContinue
    }
    ...
    # Keep the packaged setup_files intact. The completion message clearly
    # identifies the separate live installed setup_files directory.
...
function Uninstall-OutpostSeeder {
    ...
    foreach ($startupPath in @($StartupFile, $LegacyStartupFile)) {
...
function Start-VisibleScheduledSeedWorker {
    # The hidden scheduler handles timing only. Actual scheduled seeding runs in
    # a separate visible process so the user can stop that day's worker without
    # terminating the scheduler for future days.
    try {
        Start-Process `
            -FilePath "powershell.exe" `
            -WorkingDirectory $AppDirectory `
        ...
        Write-Log "Scheduled seeding worker launched in a visible window."
        return $true
    }
    catch {
        Write-Log "WARNING: Could not launch visible scheduled seeding worker: $($_.Exception.Message)"
        Write-ErrorLog -Context "Launching visible scheduled seeding worker" -ErrorRecord $_
        return $false
    }
}
    ...
    try {
        ...
        while ($true) {
            ...
            if ($withinStartWindow -and $lastRunUtcDate -ne $todayUtc) {
                ...
                try {
                    $preflightResult = Invoke-SeedingCheck -PreflightOnly
                }
                catch {
                    Write-Log "UNEXPECTED ERROR: $($_.Exception.Message)"
                    Write-ErrorLog -Context "Unexpected scheduler preflight error" -ErrorRecord $_
                }
                ...
                try {
                    ...
                    Write-Log "WARNING: Could not reload config after scheduled preflight: $($_.Exception.Message)"
                    Write-ErrorLog -Context "Reloading post-preflight configuration" -ErrorRecord $_
                }

                if (
                    $null -ne $preflightResult -and
                    $preflightResult.Reason -eq "Remote schedule moved later"
                ) {
                    $lastRunUtcDate = $null
                ...
                elseif (
                    $null -ne $preflightResult -and
                    $preflightResult.ShouldLaunch
                ) {
                    [void](Start-VisibleScheduledSeedWorker)
                }
                else {
                    Write-Log "Scheduled preflight completed. No visible seeding worker was required."
                }
...
if ($SeedWorker) {
    try {
        ...
        if (
            ...
            $runResult.HllClosedBySeeder
        ) {
            try {
                $workerConfig = Read-Config
                if ($workerConfig.ReturnToSleepAfterSeeding) {
                    [void](Request-WindowsSleep -DelaySeconds $workerConfig.ReturnToSleepDelaySeconds)
                }
            }
            catch {
                Write-Log "WARNING: Could not evaluate return-to-sleep after visible worker: $($_.Exception.Message)"
                Write-ErrorLog -Context "Visible worker return-to-sleep" -ErrorRecord $_
            }
        }
        ...
        Write-ErrorLog -Context "Visible scheduled seeding worker" -ErrorRecord $_
        Write-Host ""
        Write-Host "Scheduled seeding worker stopped with an error."
        Write-Host "See errors.txt for details."
        Read-Host "Press Enter to close"
        exit 1
    }
}
